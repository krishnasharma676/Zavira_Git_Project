      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-y-0 left-0 w-[280px] bg-white dark:bg-[#121212] z-[110] p-8 flex flex-col transition-colors duration-300"
          >
            ...
            <div className="flex flex-col space-y-6 uppercase tracking-widest text-xs font-bold text-gray-700 dark:text-gray-300">
              <Link to="/shop" onClick={onClose} className={isActive('/shop') ? 'text-[#7A578D]' : ''}>Store</Link>
              
              {/* Dynamic Categories */}
              <div className="pt-2 pb-2 space-y-4">
                <p className="text-[9px] text-gray-400 font-black">Categories</p>
                <div className="flex flex-col space-y-4 pl-2">
                  {categories.length > 0 ? categories.map((cat) => (
                    <Link 
                      ...
                      onClick={onClose} 
                      className={isActive(`/shop?category=${cat.slug}`) ? 'text-[#7A578D]' : ''}
                    >
                      {cat.name}
                    </Link>
                  )) : (
                    <>
                      <Link to="/shop?category=rings" onClick={onClose}>Rings</Link>
                      <Link to="/shop?category=necklaces" onClick={onClose}>Necklaces</Link>
                    </>
                  )}
                </div>
              </div>

              <Link to="/shop" onClick={onClose} className={`text-[#7A578D] ${isActive('/shop') ? 'underline' : ''}`}>Hot Deals 🔥</Link>
