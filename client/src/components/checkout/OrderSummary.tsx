...
const OrderSummary = ({ items, subtotal, shipping, tax, total }: OrderSummaryProps) => {
  return (
    <div className="lg:border-l lg:border-gray-100 dark:lg:border-white/5 lg:pl-16 space-y-10">
      <h2 className="text-[13px] uppercase tracking-[0.3em] font-black italic border-b border-gray-100 dark:border-white/10 pb-4">Manifest Summary</h2>
      ...
      <div className="space-y-4 pt-10 border-t border-gray-100 dark:border-white/10 text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500">
        ...
        <div className="flex justify-between items-center bg-[#7A578D]/5 p-3 rounded-xl">
          <span className={shipping === 0 ? "text-[#7A578D]" : "text-gray-500"}>Logistics {shipping === 0 ? "Exemption" : "Fee"}</span>
          <span className={shipping === 0 ? "text-[#7A578D] italic font-black" : "text-gray-900 dark:text-white"}>{shipping === 0 ? 'COMPLIMENTARY' : formatCurrency(shipping)}</span>
        </div>
      </div>

      <div className="flex justify-between items-end pt-6 border-t border-black/5 dark:border-white/5">
        <div className="space-y-1">
          <span className="text-[10px] uppercase font-black tracking-[0.4em] text-[#7A578D]">Final Allocation</span>
        ...
        <p className="text-[9px] text-gray-400 font-bold text-right uppercase tracking-widest italic leading-tight">All taxes applied per <br/>luxury standards.</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 pt-4">
        <div className="flex items-center space-x-3 text-gray-400 bg-gray-50/50 dark:bg-white/[0.02] p-4 rounded-xl border border-gray-100 dark:border-white/10">
          ...
          <span className="text-[8px] uppercase tracking-widest font-black leading-tight">Secured <br/>Logistics</span>
        </div>
        <div className="flex items-center space-x-3 text-gray-400 bg-gray-50/50 dark:bg-white/[0.02] p-4 rounded-xl border border-gray-100 dark:border-white/10">
